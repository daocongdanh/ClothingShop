...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    const fetchApi = async () => {
      try {
        ...
        setAddress(userResponse.data.address.filter(item => item.isDefault)[0].detail);
  ...
  const handleCheckout = async () => {
    if(address === null || address === ''){
      toast.error("Vui lòng nhập địa chỉ", {duration: 1500});
      return;
    }
    const data = {
      address: address,
      ...
      <div className="flex justify-between items-start text-[#333333]">
        <div className="bg-[#FFFFFF] w-[65%] border-r-[1px] border-gray-200 px-[15px]">
          ...
          <div className="flex flex-col mb-[15px]">
            ...
            <input onChange={(e) => setAddress(e.target.value)} defaultValue={user?.address.filter(item => item.isDefault)[0].detail} id="address" type="text" className="h-[45px] py-[5px] px-[20px] outline-none border-[1px] border-gray-400 font-[500] text-[14px] w-full focus:border-black rounded-[8px]" placeholder="Địa chỉ" required/>
          </div>
